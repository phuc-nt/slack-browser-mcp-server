import { SlackToolFactory } from './base.js';
...
import { GetThreadContextTool, NavigateThreadRepliesTool, CreateThreadTool, ResolveThreadTool, ArchiveThreadTool, SummarizeThreadTool, GetThreadParticipantsTool, BulkThreadActionsTool } from './thread-tool-implementations.js';
import { ThreadTools } from './threads.js';
// Import workflow tools
import { PromoteThreadTool, EscalateThreadTool, MergeThreadsTool, SplitThreadTool, WatchThreadTool, AnalyzeThreadMetricsTool } from './thread-workflow-implementations.js';
...
export class EnhancedToolFactory extends SlackToolFactory {
    ...
    registerBuiltInTools() {
        ...
        try {
            ...
            const watchThreadTool = new WatchThreadTool(ThreadWorkflowTools.createWatchThreadTool());
            const analyzeThreadMetricsTool = new AnalyzeThreadMetricsTool(ThreadWorkflowTools.createAnalyzeThreadMetricsTool());
     ...
     */
    async validateToolInput(toolName, args) {
     ...
     */
    loadToolsFromDefinitions(definitions) {
        const loaded = [];
        const failed = [];
    ...
    getToolsByCategory(category) {
        return Array.from(this.toolInstances.values()).filter((tool) => tool.getDefinition().category === category);
    }
    /**
     * Clear all caches
     */
