...
class ThreadToolHelper {
    ...
    static async getThreadReplies(client, channelId, threadTs) {
...
export class CreateThreadTool extends BaseSlackTool {
    async executeImpl(args, context) {
        ...
        try {
            const client = await ThreadToolHelper.initializeSlackClient();
            const postResponse = await client.postMessage(args.channel_id, args.initial_reply, undefined, // blocks
...
export class ResolveThreadTool extends BaseSlackTool {
    async executeImpl(args, context) {
        ...
        try {
            ...
            let resolutionMessageTs;
            if (args.resolution_summary) {
                try {
                    const postResponse = await client.postMessage(args.channel_id, `🎯 **Thread Resolved**\n\n${args.resolution_summary}`, undefined, // blocks
...
export class ArchiveThreadTool extends BaseSlackTool {
    async executeImpl(args, context) {
        ...
        try {
            ...
            let archiveMessageTs;
            if (args.archive_reason) {
                try {
                    const postResponse = await client.postMessage(args.channel_id, `📁 **Thread Archived**\n\nReason: ${args.archive_reason}`, undefined, // blocks
                    args.thread_ts, // thread_ts
                    undefined, // attachments
                    false // unfurl_links
                    );
...
//# sourceMappingURL=thread-tool-implementations.js.map